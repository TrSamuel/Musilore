...
class MusicContainer extends StatelessWidget {
  ...
  const MusicContainer({
    super.key,
    required this.darkTheme, required this.secondaryColor,
  });
  ...
  Widget build(BuildContext context) {
    return Consumer<SongNotifier>(
      builder: (context, songPLayNotiferData, _) {
        final Uint8List? image = songPLayNotiferData.currentPlayAudioModel!.image;
        return Container(
          width: fullsize(context: context).width * 0.7,
          height: fullsize(context: context).width * 0.7,
          decoration: BoxDecoration(
              ...
              boxShadow: [
                BoxShadow(color: secondaryColor, blurRadius: 50)
              ]),
          child: image == null
              ? Center(
                  child: Icon(
                    color:
                        darkTheme ? primaryTextColor : secondaryColor,
                    size: 150,
